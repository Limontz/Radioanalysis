...
class Image(object):
    ...
    def __init__(self, imagefile):
        ...
        self.imagename = self.imagefile.split('/')[-1]
        ...
        try:
            ...
            logging.warning('%s: No beam information found.' % self.imagename)
            beam = [None, None, None]
            # sys.exit(1)
        logging.debug('%s: Beam: %.1f" %.1f" (pa %.1f deg)' % \
                (self.imagename, beam[0]*3600., beam[1]*3600., beam[2]))

        self.freq = find_freq(header)
        if self.freq is None:
            logging.warning('%s: No frequency information found.' % self.imagename)
            # sys.exit(1)
        else:
            logging.debug('%s: Frequency: %.0f MHz' % (self.imagename, self.freq/1e6))
        try:
            self.mhz = np.round(self.freq / 1e6).astype(int) # for suffixes etc
        except TypeError:
            self.mhz = None
    ...
    def write(self, filename=None, inflate=False):
        ...
        if inflate:
            # R.I.P. I found no other way than explicitly hardcoding this, some sw (BRATS especially) complained about headers otherwise...)
    ...
    def apply_region(self, regionfile, blankvalue=np.nan, invert=False):
        ...
        logging.debug('%s: Apply region %s' % (self.imagename, regionfile))
    ...
    def apply_mask(self, mask, blankvalue=np.nan, invert=False):
        ...
        """
        logging.debug('%s: Apply mask' % self.imagename)
        if invert: self.img_data[~mask] = blankvalue
        else: self.img_data[mask] = blankvalue
    ...
    def blank_noisy(self, nsigma):
        ...
        print(nsigma, self.noise)
        self.img_data[np.where(self.img_data <= nsigma * self.noise)] = np.nan
        nans_after = np.sum(np.isnan(self.img_data))
        logging.debug('%s: Blanked pixels %i -> %i' % (self.imagename, nans_before, nans_after))

    def calc_noise(self, niter=1000, eps=None, sigma=15, bg_reg=None, betaSigma=False):
        """
        ...
        bg_reg : str. If ds9 region file provided, use this as background region
        betaSigma : bool. if ture, use betaSigma to estimate noise
        """
        if bg_reg:
            import pyregion
            ...
            logging.debug('%s: Apply background region %s' % (self.imagename, bg_reg))
            ...
            logging.info('%s: Noise: %.3f mJy/b' % (self.imagename, self.noise * 1e3))

        elif betaSigma:
                """
                Use the betaSigma method "A posteriori noise estimation in variable data sets." Czesla et al.
                Parameters
                ----------
                ...
                Returns
                -------
                std: estimate of noise standard deviation
                """
                from PyAstronomy.pyasl import BSEqSamp
                data = self.img_data.flatten()
                bseq = BSEqSamp()
                smad, dsmad = bseq.betaSigma(data, 1, 20, returnMAD=True)
                return smad
        else:
            ...
            for i in range(niter):
                ...
                if np.abs(mad_old - mad) / mad < eps:
                    ...
                    logging.debug('%s: Noise: %.3f mJy/b (data len: %i -> %i - %.2f%%)' % (
                    self.imagefile, self.noise * 1e3, initial_len, len(data), 100 * len(data) / initial_len))
                    return rms
    ...
    def regrid(self, regrid_hdr, parallel=True):
        ...
        self.img_data, __footprint = reproject_exact((self.img_data, self.img_hdr), regrid_hdr, parallel=parallel)
        beam = self.get_beam()
    ...
    def pixel_covariance(self, pix1, pix2):
        ...
        pix1, pix2 = np.round(pix1), np.round(pix2)
        dx, dy = pix1 - pix2
        ...
        if np.linalg.norm([dx, dy]) > 5*b_sig_pix_ma:
            Cij = 0.
        else:
