...
def interpolate_path(region, image, z, flux_scale_err=0.0, offset=0.0):
    """
    ...
    if False: # set this to true to get the interpolated path as circles, can turn to segment and whatever manually...
        # fit path and fill data frame with fitted points and distance of points
        df = pd.DataFrame()
        xy, l = fit_path_to_regions(region, image, z, 1/3600)
        df['l'] = l + offset
        radec = image.get_wcs().all_pix2world(xy, 0)  # TODO check origin
        df['ra'], df['dec'] = radec.T

        # write interpolated region -- this is for plotting purposes.
    ...
    if False: # Set this to true to get the beam sized regions used for the analysis.
        ### Start section only for plotting!!
        # write interpolated region -- this is for plotting purposes.
        line = 'global color=#ababab dash=0 fixed=0 edit=1 move=1 delete=1 include=1 source=1 \nfk5'
        if offset == 0.0:
            n_pt, last_pt = 0, -1
    ...
    for i,p in enumerate(radec):
        ...
        path_data_error[i] = ((image.noise * np.sqrt(npix / image.get_beam_area(unit='pixel')))**2 + (path_data_psf[i]*flux_scale_err)**2)**0.5
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fluxerr', default = 0.0, type=float, help='Flux scale error of all images. Provide a fraction, e.g. 0.1 for a 10% error.')
    ...
    l_sel = (0 <= df['l']) & (df['l'] - args.injection_offset <= 240)  # length selection
    ...
    def residual_SI_aging_path(param):
        ...
        Parameters
        ----------
        p: object, input parameters [v] where v velocity in km/s
        ...
        for id_d, d in enumerate(X[:, 0]):
            for id_nu in range(len(X[0]) - 2):
                id_nu += 1
                map_args.append(
                    [X[id_d, id_nu], X[id_d, id_nu + 1], B_min, args.iidx, d / (param['v'].value * kmpers_to_kpc_per_Myr), args.z, S])
        res = np.array(list(p.starmap(lib_aging.get_aging_si, map_args)))
        return (res.reshape((len(X), len(X[0]) - 2)) - Y) * (1/Yerr)**2 # 1 = Y
    ...
    if args.reuse_fit and os.path.exists(f'{args.out}-fit.pickle') and os.path.exists(f'{args.out}-fit-norms.pickle'):
        ...
        mini = lmfit.Minimizer(residual_SI_aging_path, params, nan_policy='propagate')
    ...
    velocity = result.params['v']*kmpers_to_kpc_per_Myr
    ...
    for j, im in enumerate(all_images):
        ...
        ax[0].errorbar(df['l'], df[f'F_{im.mhz}']*scale, df[f'F_err_{im.mhz}']*scale,
                       uplims=df[f'F_{im.mhz}'] < 3*df[f'F_err_{im.mhz}'], color=cs[j],
    ...
    for i, (image1, image2) in enumerate(zip(all_images[:-1], all_images[1:])):
        ...
        df[f'SI_model_{image1.mhz}-{image2.mhz}'] = lib_aging.get_aging_si(image1.freq, image2.freq, B, args.iidx, df['l'] / velocity, args.z, S)
