    ...
    for n, fgs in enumerate(fgss):
        ...
        for fg in fgs:
            for i in range(fg.rm.nchans):
                freq = fg.rm.frq[i]
                print(f"{n}: {fg.rm.filename} {freq:8.4g}MHz flux={fg.flux[i]:10.6g} +/-{(fg.error[i]**2 + ((fluxerr/100)*fg.flux[i])**2)**0.5:10.6g}")#Jy %8.4g %10.6g %10.6g' % (freq,fg.flux[i],fg.error[i]))
