...
ngalaxiesfile = main + '/File/total_ngalaxies.csv'
ngalaxiesdf = pd.read_csv(ngalaxiesfile, delimiter=',', usecols=('name', 'ngalaxies'))

df = pd.merge(df, ngalaxiesdf, on=['name'], how='left')
...
plt.scatter(df.lls, 1/(df.power/(df.ngalaxies)), marker='.', s = 100, edgecolors='black', facecolors='blue', label='HERG')
