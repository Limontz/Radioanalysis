...
main = '/Users/marco/Desktop/The_Master/PhD/GRG Project/LHLDF/File/'
rgfile = main + 'LH_RGs_catalogue.csv'
file = main + 'apt_LH_rgs.csv'
...
for i in range(len(df.name)):
    ...
    images = [main+'spidxmap/hba/'+str(df.name[i])+'_hba.fits', main+'spidxmap/apt/'+str(df.name[i])+'_apt.fits'] 
    output = main+ 'spidxmap/output/apt-hba/' + str(df.name[i]) + '_spidx.fits'
    radec = [df.ra[i], df.dec[i]]
    # if (i == 20 or i ==27):
    #    size = [df.las[i]*2/60, df.las[i]*2/60] 
    # else:   
    #    size = [df.las[i]*1.5/60, df.las[i]*1.5/60]
    size = [df.las[i]*1.5/60, df.las[i]*1.5/60]
    # check input
    if len(images) < 2:
    ...
    #########################################################
        ...
        spidx.writeto(output, overwrite=True)
        spidx_err.writeto(output.replace('.fits','-err.fits'), overwrite=True)
